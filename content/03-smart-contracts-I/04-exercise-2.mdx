...
* **payOff**: The project initiator can transfer the collected funds to a different address. This can only be done if the **funding target is met**. For reasons of simplicity, the owner can call this function only after the deadline has passed.
...
* **minAmount**: This is the minimum limit to be achieved for a crowdfund to be considered successful.
...
```@addTest(name = "successful_Crowdfunding")
def successful():
...
* Crowdfunding participants to request a refunde, if the funding target isn't met,
* The storage to contain the **minAmount** and **maxTime**.
