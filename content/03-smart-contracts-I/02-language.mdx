---
title: SmartPy library
description: SmartPy library
slug: /smartpy/language
---

# SmartPy - Language

Now, let us write a different smart contract, which sums all natural numbers from 0 to a given number: 
...
def test():
    ...
    secondContract= summarize()

    scenario+=secondContract
    scenario+= secondContract.sum(5)
```
...
How does the Michelson code of such a contract look?
...
Can you see how the loop is [unrolled](https://en.wikipedia.org/wiki/Loop_unrolling) and Python appends the addition inline?
...
This means that the loop as such is maintained in Michelson. In doing so, we can see and decide which calculations we want to perform on the blockchain.
...
When we take a more detailed look in the **SmartPy** editor at the tab **Types**, we see:
...
Our storage has the type `TIntOrNat`. This is due to the fact that **Michelson** has the types `int` and `nat`, and our script does not allow **SmartPy** to simply recognize which one we need or respectively which one we want to use. When observing the **Michelson** code, we can see that our storage in the end thus has the type **int** - eventually a decision has to be made.
...
def test():
    ...
    scenario.register(firstContract, show = True)
    scenario+= firstContract.repeat(-1).run(valid=False)
```
...
We also have `sp.if`. So, when we do not want our script to stop, we can write:
...
def test():
    ...
    scenario.register(firstContract, show = True)
    scenario+= firstContract.repeat(-1)
```

We have used `sp.for` already. In addition, **SmartPy** offers `sp.while`.
...
<div class="b9-tip">
<p>Take a look at this <a href="https://smartpy.io/dev/reference.html">SmartPy reference</a>. It contains a very useful overview.</p>
</div>
