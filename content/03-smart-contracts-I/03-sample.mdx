---
title: SmartPy Sample
description: SmartPy Sample
slug: /smartpy/sample
---

A smart contract to certify students
Let us test a more complicated storage structure. At the end of this course, we will test the students, and accordingly certify their performance. Let us write the for this purpose needed basic smart contract together.

Let us begin our thinking with what we want:
...
will lead to an invalid transaction.
...
Now, we can deal with the second issue. We need a [unique identifier](https://en.wikipedia.org/wiki/Unique_identifier) for the students. The most simple and obvious solution would be the use of the student's Tezos address. For this, a `map` is perfectly suited:
...
Finally, we want to use a bigMap instead of a map:
...
SmartPy allows us to deploy the smart contract directly within the browser. First, we can again import an account from the faucet:
