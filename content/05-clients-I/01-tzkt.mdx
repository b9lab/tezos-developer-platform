...
You can specify `select`, and use `select.fields` or `select.values` to fetch the fields or the values. As you can see in the `request`, we can also tell TzKT API to `sort` the balances. This will give us 10 richest accounts.
...
TzKT offers [deep selection](https://baking-bad.org/blog/2021/03/03/tzkt-v14-released-with-improved-smart-contract-data-and-websocket-api/#get-specific-fields-from-the-storage), so we do not need to fetch the whole storage if we are interested to know the certifier:
...
This will wait `timeBetweenBlocks` from the timestamp of the last recent block before to fetching the timestamp of the most recent block. Also, even after the time is passed, it won't do more than one request each 5 seconds. This should be enough to have an accurate storage.
...
Let us do something else not easy to do without the TzKT API and ask for the [number of smart contracts](https://api.tzkt.io/#operation/Accounts_GetCount) in the edonet:
