...
We need a possibility for the average user to interact with our smart contract. It is similar to the student certification smart contract, which you already deployed. We do not store the name of the students to keep our first client simple.
...
There is a npm package for [Taquito](https://www.npmjs.com/package/@taquito/taquito) that we can use.
...
We are taking the primrose path: We will first create a HTML page, and with it contact a public server to fetch data from the Tezos blockchain.
...
You can [download this DApp](repolink) and test it. The smart contract is hardcoded at the address KT1SgX1ZehTysGcNoSwypwg6Xs5Cj7cpokwK, and we added a student with the address tz1Q2zkgZENNF2g95NN7g1CtxAqKynWViSeN.
...
In the present case, we only need to pass the address of the student to certify, which is easily done. Taquito extracts the entrypoints from the contract and allows us to do an usual call on those.

The testnet network processes transactions with a 0 fee, but with this wallet from the faucet we are rich. 

This time, to be able to pack all the necessary information, we need more than one input field:
...
or download it from the [certifier](repolink) repository.
