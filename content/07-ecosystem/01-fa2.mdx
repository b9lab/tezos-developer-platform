...
We have not used the metadata standard for Tezos smart contracts, [TZIP-16](https://gitlab.com/tzip/tzip/-/blob/master/proposals/tzip-16/tzip-16.md), so far [with SmartPy](https://smartpy-io.medium.com/metadata-in-smartpy-3eb09fcd1de7). Basically some data can be stored off-chain with a link on-chain (as `big_map %metadata string bytes`) to be easier accessible. 
