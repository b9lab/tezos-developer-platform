...
We first call for a `requestPermissions` for the Florencenet testnet, fetch the address of the user, and print it into the console. Then we **set the wallet** as our provider.
...
Now we can **set the network** to Florencenet:
