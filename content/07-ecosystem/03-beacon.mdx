...
A wallet will store your keys and can also fulfill other tasks like transaction signing. 

[Beacon](https://www.walletbeacon.io/) is an interface between such a wallet and your application. At the time of writing, Beacon supports [AirGap](https://airgap.it/), [Galleon](https://github.com/Cryptonomic/Deployments/wiki/Galleon:-Releases), [Kukai](https://wallet.kukai.app/), [Thanos](https://templewallet.com/) and [Spire](https://chrome.google.com/webstore/detail/spire/gpfndedineagiepkpinficbcbbgjoenn). 

We have different options how we can use the [Beacon SDK](https://github.com/airgap-it/beacon-sdk). We will work with Taquito's [Wallet API](https://tezostaquito.io/docs/wallet_api/) and extend our [certifier application](https://github.com/b9lab/tezos-sample-certifier) so it can be used with a wallet. This is possible because there is a wallet interaction specification called [TZIP-10](https://gitlab.com/tzip/tzip/-/blob/master/proposals/tzip-10/tzip-10.md) which is implemented by Beacon.

# Wallet API

Wallet API supports different wallets, the npm package for Beacon is `@taquito/beacon-wallet`. 
...
<expendable>
and a bit styling so it does not hurt our eyes:
...
Ok so far we just removed the input fields for the faucet account. Instead we now have a button to connect to a wallet. Now you need to rewrite parts of `certifier.js`:
...
// This is function will connect your application with the wallet
...
// This is the main function, interacting with the contract through Taquito Wallet API
...
There are other [usefull options](https://tezostaquito.io/docs/wallet_api/#--tzip-10-wallet) like `eventHandlers` which should be utilized for better user exprience in a real application. Anyway we will keep it simple and set only what must be set. 
...
Now the wallet will do the signing of our transactions:
...
As you can see it is just slightly changed, we use `tezos.wallet.at` instead of `tezos.contract.at` to get the contract abstraction. Let us test this.
...
In this case you can setup a local secret and you do not need a wallet to pair with. 
...
Now let us start our application. Run in your project folder:
...
it will tell you where the server is running so you can test your application. In addition show the console to see the outputs:

![](images/beacon4.png)

Now when you hit the connect button, you most likely will see in the console something smilar to:
...
this is an issue with Parcel we need to fix first, in your project folder run:
...
Now you should see your address in the console.
...
Unfortunately at the time of writing, we cannot import faucet accounts into Spire. Instead it will generate a new account with 0 tez in it. 

So to continue, first you will need to transfer some tez from a faucet account on the testnet to your address in Spire. You can do this like we did in the CLI section with the `tezos-client` or via [SmartPy](https://smartpy.io/wallet.html). You will need to visit SmartPy anyway to deploy the [certification contract](https://smartpy.io/ide?code=eJytkUFLxDAQhe_5FaGnFCUguhehoO5F8SKoiIiEmKQ0uE1jZna1__D75A7Mw9njaXOr35cL_dRsrj@_lGvun9Xio8Wr2hXlOBSUcTLYOTqX2LT1_H9UZfg2zr7G82Okq@VMJQ_rmHFc_V@jHmDFJf0X6gddefug) again for your new address.

After that you can try to issue a certificate. Spire will pop up and let you confirm the transaction:

![](images/beacon8.png)

<reading>
https://medium.com/coinmonks/build-a-tezos-dapp-using-taquito-and-the-beacon-sdk-692d7dc822aa
https://medium.com/tezos-israel/building-a-simple-react-d-app-on-the-tezos-blockchain-part-1-777ff6b0b84e
</reading>
