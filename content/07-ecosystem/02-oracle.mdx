...
It inherits the classes `Client_requester` and `Client_receiver`. This simplifies things because we need to deploy one contract less. `Client_requester` can send and pay requests to oracle via the escrow contract. The `Client_receiver` expects to be called by the oracle contract.
...
There you see the requests list `requests = sp.big_map(tkey = sp.TNat, tvalue = TRequest)`. At the time of writing, each data provider will need to deploy an oracle contract, so we do not have any proxy. This contract has two important methods:
...
Finally let us have a look the escrow contract comments:
...
So client will call escrow, which will forward a request to the oracle. Beacuse the escrow contract handels the payments and validation, it increases the trust in the system. 

Let us see this in a test from the template:
...
Those call are the calls you will need to make with your clients if all contracts are deployed. 

Data provider will:

- check the storage of Oracle contract
- process a request and call `fulfill_request` on the Oracle contract

Data seeker will:

- call `request_value` with the amount of token and timeout parameters
- check the storage for the requested value

<HighlightBox type="tip">
You will also need to deploy a faucet to get some tokens to test this. Have a look at the [Writing Smart Contracts](..smartpy/exercise-1/) section to see how you can deploy those contracts with SmartPy and a look at the [Developing Clients](../taquito) section to see how you can fetch the storage and call the entrypoints of deployed contracts.
</HighlightBox>
...
<HighlightBox type="tip">
There are allready contracts deployed for [Coinbase](https://tzkt.io/KT1Jr5t9UvGiqkvvsuUbPJHaYx24NzdUwNW9/storage) - signed by Coinbase Pro, [Binance](https://tzkt.io/KT1Mx5sFU4BZqnAaJRpMzqaPbd2qMCFmcqea/storage), [Gemini](https://tzkt.io/KT1Jud6STRGZs6hSfgZsaeztbkzfwC3JswJP/storage) and [OKEx](https://tzkt.io/KT1G3UMEkhxso5cdx2fvoJRJu5nUjBWKMrET/storage) - signed by Blockscale on the mainnet. You can see the public keys of the signer in the storage. Notice that only a signer can update a price feed. 
</HighlightBox>

Then you can use the [Harbinger Serverless Price Feed Signer](https://github.com/tacoinfra/harbinger-signer) to sign price feeds. The CLI can also update the contract with signed price. In addition you can use [Harbinger Serverless Poster](https://github.com/tacoinfra/harbinger-poster) which can update the price automatically.

<HighlightBox type="tip">
If you do not want to use those high level tools, have a look at the [harbinger-lib](https://github.com/tacoinfra/harbinger-lib) repository.
</HighlightBox>
...
<HighlightBox type="tip">
Have a look at the [Developing Clients](../taquito) section to see how you can fetch the storage. Notice that it might be better to fetch the rates from the normaliser contract because of the volume weighted average price it contains. 
</HighlightBox>
