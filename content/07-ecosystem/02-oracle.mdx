...
The term **[oracle](https://en.wikipedia.org/wiki/Oracle_machine)**, in computability theory, describes an instance which can solve problems that a turing machine is not capable of. In blockchain, we have a similar situation where the term oracle appers.
...
You can find a template for Chainlink oracles by SmartPy [here](https://smartpy.io/ide?template=oracle.py), and there also is the offical [Solidity implementation](https://github.com/smartcontractkit/chainlink/blob/master/evm-contracts/src/v0.6/Oracle.sol). You can also take a look at the [Chainlink Developer Ressources](https://docs.chain.link/docs/getting-started), if you want to dive deeper.
...
For many DeFi applications to run seemesly a **trustworthy and reliable price feed** is vital. Among others, a price feed allows for derivatives, futures smart contracts, insurance, etc.

Why is a price feed so important?

It comes down to a question of **trust**. As DeFi applications involve financial means and their value stemms strongly from the accuracy, reliability, and safety of price data, it is important to integrate a price oracle on protocols like Tezos. This avoids (maliciously) manipulated data creating unreliable and untrusty price feeds.
...
It allowes you to fetch signed price feeds from market data. Exchanges, such as Coinbase or OKEx can let their oracles submit a signed price feed, which is then included on-chain by a so-called poster. The exchange signes the price with their private key.
...
An exchange provides a **market data API**, which responses with a price when queryed. Price data usulally comes from an exchange's market order book.
...
**Or**, the poster invokes a callback on the storage contact pushing the data to a **normaliser contract**. The **volume-weighted average price** is then calaculated by the latter.
...
By using the reference signer, security is increased while it deploys independently from the excahnge's network and servers, doesn't have to be maintained, and has a secure caching API gateway. Posters cannot request a price from the APIs more than once per minute.

Now, let's turn to the **poster**!

To provide reliable and updated price data, posters get signed prices from either a signer or directly from an exchange. The poster can post the prices on-chain at any time. Updating prices is thus decentrally organised - everyone can post price data from multiple sources and posting doesn't have to rely on just one poster.

As mentioned before, the price is only accepted by the storage contract if the data is signed with a private key.

Posters can function as Serverless Framework applications or run a command line as part of their process to get price data in continous intervals.
...
The price data is availabale in so-called *data candles** (i.e. data points for a digital asset). Each candle includes:
...
Protocols and applications can also invole a callback on a storage or normaliser contract. By doing this, data is pushed to the protocols or applications contract.

<HighlightBox type="tip">

When developing on Tezos, use the callback to get data to avoid reentrancy attacks.
...
* As a standard unix timepstamp for a candle start and end, or
...
* the **number of data points** being stored and normlised.
...
* Luke Youngblood (2020): [Introducing Harbinger: a Self-Sustaining Price Oracle for Tezos_ (https://medium.com/@Blockscale/introducing-harbinger-a-self-sustaining-price-oracle-for-tezos-7cab5c9971d)
