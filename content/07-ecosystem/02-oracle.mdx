...
There you see the requests list `requests = sp.big_map(tkey = sp.TNat, tvalue = TRequest)`. At the time of writing, each data provider will need to deploy an oracle contract, so we don't have any proxy. This contract has two important methods:
...
Let's see this in a test from the template:
...
You will also need to deploy a faucet to get some tokens to test this. Have a look at the _[Writing Smart Contracts](..smartpy/exercise-1/)_ section to see how you can deploy those contracts with SmartPy and a look at the [Developing Clients](../taquito) section to see how you can fetch the storage and call the entry points of deployed contracts.
