...
As said before, in the **rolling mode**, the minimal data for rolling is stored. Thus, it is the _lightest mode_. The node is able to validate blocks and only requires minimal and bounded disk storage. Bootstrapping the mode can be done within minutes. It is suitable to run on low-resource architectures. At the same time, the node cannot query block information in regard to balances and staking rights. Furthermore, it doesn't help other nodes to bootstrap because it cannot provide the complete chain history.

If you want to take a closer look at how to **set up a rolling node**, we recommend the [documentation on setting up a node in rolling mode](https://tezos.gitlab.io/user/history_modes.html#set-up-a-rolling-node).

**Full mode** for nodes allows to store the minimal data needed to reconstruct the ledger state, thus requiring little disk storage. It can be synchronised using a snapshot, and can help other nodes bootstrap and synchronise with the Tezos chain. This mode is most-suitable for baker nodes. The necessary

The **full mode** 


It can be synchronised with a snapshot and doesn't require much disk storage. 
