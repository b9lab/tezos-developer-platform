...
* the proposal period,
* the exploration vote period or testing_vote period,
* the testing period,
* the promotion vote period, and
* the adoption period.
...
**Adoption period:** Introduced with the Edo upgrade, this period is meant to provide enough time for tool providers and other ecosystem participants to prepare for the changes being introduced by a proposal. The aim is to enable a stable protocol transition for the protocol itself and its ecosystem. At the end of this period, the proposal is activated. Adding this last phase through the Edo upgrade represented a **proof of resilience and the evolving nature of the mechanism**, as it was the first amendment to the process itself. 
</ExpansionPanelListItem>
</ExpansionPanelList>
</ExpansionPanel>

All the above mentioned periods lasts for **five baking cycles** - the length of the amendment process was reduced from eight to five cycles by the _Edo upgrade_.

<HighlightBox type="info">

What is a cycle? Cycles are used as a term in Tezos to indicate **time** measurements.

How do cycles relate to know time measurements? One cycle equals to a total of **4,096 blocks**. As the time between the creation of two blocks is fixed to **1 minute**, a cycle lasts approximately 4096 minutes, i.e. two days, 20 hours, and 16 minutes. Remember that the time between two blocks can vary.

</HighlightBox>

When a period is successful, the amendment process is finalised. In case it is not, the proposal period begins again, the process restarts so to say.
...
Another attribute of Tezos is its consensus mechanism, **Liquid Proof-of-Stake (LPoS)**. Compared to a DPoS mechanism, LPoS prioritises security, decentralisation, and accountability in governance. Let's take a closer look at LPoS!

In an LPoS-based blockchain, a **baker**, i.e. validator node, is **randomly selected** to generate blocks based on its **own funds staked** and those that are **delegated** to it by other stakeholders. Whereas, the **stake** is measured in Tezos tokens (XTZ), also called **tez**. Afterwards, randomly selected endorsers validate blocks produced by bakers.

Block creation rights are randomly selected proportional to a baker’s stake, for performance reasons measured in "rolls". Currently, one **roll** is equal to 8,000 tez, rounded down (e.g. 15,999 tez equals one roll) - This constant may be changed via the Tezos governance mechanism. The rolls available to a baker consist of the bakers own staked tez and the delegated tez from originated accounts. Bakers are required to put up a _security deposit_ to disincentivise malicious behaviour. The size of a baker's deposit depends on the amount of blocks a baker is selected to produce and/or endorse.

Bakers earn **rewards** for securing the network through the generation and endorsment of blocks. Rewards are currently [dynamically calculated](https://blog.nomadic-labs.com/emmy-an-improved-consensus-algorithm.html) and were initially calibrated for a target inflation rate of approximately 5.5%.
...
As mentioned, stakeholders that do not bake themselves (via an implicit account) can delegate their stakes to bakers. This allows stakeholders of any size to participate in LPoS and earn associated rewards for securing the network. The rewards paid by bakers to delegates are outside the protocol's scope. Thus, it is up to bakers to distribute their rewards to delegates. It is expected that bakers will compete for delegated tez on the basis of reliability, fees, security, and other factors like contributions to the Tezos ecosystem.

<HighlightBox type="info">

At the time of writing (February 2021), there are [over 400 participants in Tezos PoS](https://tzstats.com/bakers). MyTezosBaker, a popular curation site for Tezos validators (referred to as **bakers** in Tezos), currently lists more than 100 public delegation services from over 30 countries across 6 continents. Check out this [site](https://mytezosbaker.com/). These are good indicators for the network stability and security.

</HighlightBox>

As Tezos stakeholders have the ability to participate in consensus either directly themselves or to delegate their stake to other bakers. As such, Tezos consensus can be described as "proof of stake with delegation". Stakeholders can fluidly switch between participating themselves or delegating, and can also easily switch delegators. For these reasons, LPoS has gained traction when describing Tezos’ consensus mechanism.
