...
Cycles are used as a term in Tezos to indicate **time** measurements. One cycle equals to a total of **4,096 blocks**. As the time between the creation of two blocks is fixed to **1 minute**, a cycle lasts approximately 4096 minutes, i.e. two days, 20 hours, and 16 minutes. Remember that the time between two blocks can vary.
...
In an LPoS-based blockchain, a **baker**, i.e. validator node, is **randomly selected** to generate blocks based on its **own funds staked** and those that are **delegated** to it by other stakeholders. Whereas, the **stake** is measured in Tezos tokens (XTZ), also called **tez**.
