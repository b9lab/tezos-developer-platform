...
You see that the response is a JSON which we can parse. Now the question appers, if we want to track the activity on our smart contract, [how often](https://baking-bad.org/blog/2020/07/29/tezos-explorer-api-tzkt-how-often-to-make-requests/) do we need to do this request?

You know that each transaction needs to be included into a block. So we do not need to fetch data more often than blocks apper. For the testnet we can assume, that every 30 seconds we will get a new block. Notice, usually you would also wait more than one block to confirm a transaction.
