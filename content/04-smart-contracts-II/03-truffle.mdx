...
As we have seen, preparing and deploying contracts can be slow and painful. We need a tool with a faster turn around time. ConsenSys's [Truffle](https://www.trufflesuite.com/truffle) is such a tool. It was first introduced for the Ethereum protocol but it supports now different protocols.
Truffle will compile your contracts and deploy them on the Tezos node of your choice.

## Install

Let us start by installing Truffle for Tezos. 
...
Tezos offers a sample project for Tezos: [Tezos Truffle Box](https://github.com/truffle-box/tezos-example-box)

Let us use it for a quick start. Create a folder for this sample and run in it:
...
- folder migrations/, where you define the ordered set of scripts that build, and update, your system
- file truffle.js truffle-config.js, where Truffle is directed as to what to do
...
Make sure that you local node is not listening at port 8732 and run:
...
Now let us [migrate](https://www.trufflesuite.com/docs/tezos/truffle/getting-started/deploying-tezos-contracts) the sample contracts:
...
Oh we have a problem with the `SimpleStorage` test. This is the right time to have a look into the test:
...
We see that it calls `simpleStorageInstance.default(89)` to store the value `89`. As you can expect `default` entrypoint of the `simpleStorage`contract will be `main`, which we can call directly. So change the regarding line into:

```javascript
    await simpleStorageInstance.main(89);
```

to fix the test. You can run the test for `simpleStorage` separately: 
...
<div class "b9-tip">
Tests do not interfere with your instances deployed via truffle migrate. Yes, tests use the same migrations/* files, but precisely to deploy new instances.
...
You can have as many test files as needed. Each test file can have as many tests as needed. Test files are structured in the following way.
...
Let us see how we can interact with the contracts.
...
Here, we are in, wait for it, the Truffle console. It is a similar Javascript environment and provides some auto-completion with the **TAB** key. We can basically do some testing in a more interactive way:
...
let us have a look into `simpleStorageInstance`:
...
There is a lot of information. You will find the Michelson code of the **repeater** contract which does the same as the `SimpleStorage` contract in the Tezos Truffle Box. In addition you can see the `methods` we can call. Let us first fetch the storage:
...
and save the students contract as `students.ligo` in the `contracts` folder of the initiated project. You will also see the migrations contract which keeps the record of the successfull compilations.
...
you can save this as `2_deploy_students.js` in the `migrations` folder. We use as certifier Alice's key from the `accounts.js`.
