...
As we have seen, preparing and deploying contracts can be slow and painful. We need a tool with a faster turnaround time. ConsenSys's [Truffle](https://www.trufflesuite.com/truffle) is such a tool. It was first introduced for the Ethereum protocol but it supports now different protocols.
...
We see that it calls `simpleStorageInstance.default(89)` to store the value `89`. As you can expect `default` entry point of the `simpleStorage`contract will be `main`, which we can call directly. So change the regarding line into:
