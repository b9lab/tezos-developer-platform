...
We see that it calls `simpleStorageInstance.default(89)` to store the value `89`. As you can expect, the `default` entry point of the `simpleStorage`contract will be the function `main`, which we can call directly.
