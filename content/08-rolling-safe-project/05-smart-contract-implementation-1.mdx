...
Next, the contract does a **reserve calculation** to determine the amount of the excess, which is sent back to the transaction sender. In addition each time someone buys a token, a part of the tez amount in the transaction will be kept for the buyback reserve:
...
A customer can call the `pay()` entrypoint to send a payment to the organization via the smart contract. If `pay()` is called, the smart contract will issue new tokens and send them to the organization. A pre-determined part of the received tez will be kept in the reserve and the rest will be sent to the organization. You can see, it will mint the same number of tokens as if `buy()` would have been called. A part of the tez sent with the call will be kept in the contract.

We will allow payments only in the slopy phase:
