...
A user can call the `pay()` entrypoint to send a payment to the organization via the smart contract. If `pay()` is called, the smart contract issues new tokens and by default send them to the organization. Optionally, the user can pass an address to which the tokens are sent. A pre-determined part of the received tez is kept in the reserve and the rest is sent to the organization.
