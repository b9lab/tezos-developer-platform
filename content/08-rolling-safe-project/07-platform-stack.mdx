...
As outliend in the [system architecture section](/rolling-safe-system-architecture), we use [Next.js](https://nextjs.org/) as server framework for the web application. Next.js is a React framework build on nodejs, which runs your application server and allows for a very modular and dynamic composition and configuration of our application. It understands (but does not enforce) an [MVC](https://developer.mozilla.org/en-US/docs/Glossary/MVC) approach, routing files, data connections, server/client side rendering and the use of (react) components in the UI.
...
You can create and add further components like this, if you want to see an example for passing multiple custom properties to a component, you can take a look at the [CtaCard Component](https://github.com/b9lab/tezos-cso-project/blob/master/src/components/CtaCard.tsx).
...
which then calls `companyValuation()` on the `chainWrapper`. We can call use this function directly from our React components. To extend our initial Home page sample with this, we first need to import the DataHandler, using:

```javascript
import DataHandler from "../src/services/DataHandler";
```

Then, to fetch data from it, we instantiate a new DataHandler object and call the `getCompanyValuation` function:
...
However, since this is calling an asynchronous function, we can not run this within the component constructor. Next.js provides a specific function for this case, which fetches the data on the server:
...
Next.js supports different ways to actually render your data. So far, we have seen how to use [Server-side rendering (SSR)](https://nextjs.org/docs/basic-features/data-fetching/get-server-side-props), which fetches the data on the server on each request, and then renders the page with it.

Alternatively, Next.js also allows you to use [Static-site generation (SSG)](https://nextjs.org/docs/basic-features/data-fetching/get-static-props), which fetches the data **during build time**, and then renders it as static value. To use SSG, you can use the same approach as demonstrated for SSR, but name the function `getStaticProps(context)` instead of `getServerSideProps(context)` - that's it!

Finally, we can also implement Client-side rendering. For this, Next.js recommends to use [SWR](https://swr.vercel.app/docs/with-nextjs), which is a React hook library created by the Next.js team exactly for this purpose. Using SWR requires an additional modification: It wants to read the data from our server via a REST Api, sending JSON data. So, we have to create an Api endpoint in our Next.js server, wrapping our data fetch call and returning a proper HTTP response. We add a new file in `/pages/api/company-valuation.ts` with:
...
This api will then be exposed on `/api/company-valuation`. To integrate this in our page, Next.js supplies a special function `useSWR`, which is **not asynchronous**, so we can call it in the constructor. Our home page with Client-side data fetching:
...
Using SWR also allows to combine Server-side and Client-side rendering: You can use SSR or SSG to pre-render the page content, then update it client-side. We are using this for the page displaying the Cafe parameters within our application. These parameters are configured on the smart contract during deployment. Next.js will fetch them during build, and then check for an update from the client, but after the page has been rendered. For this, we first supply the SSG function `getStaticProps` as before:
```javascript
...
Then,we pass this data as second parameter to the `useSWR()` function, which wraps all the update magic.
...
Interaction with the user's wallet is done through the wallet wrapper ([/src/wallet/wallet.js](https://github.com/b9lab/tezos-app-project/blob/main/src/wallet/wallet.js)), which wraps the Beacon SDK and passes the configuration to initialize it. The code is very similar to what we have seen in the [Beacon Section](/beacon) in the previous module:
...
The actual instantiation and configuration of the options happens in the main export file of the project, the file [index.ts](https://github.com/b9lab/tezos-app-project/blob/main/index.ts) in the projects root folder:
...
We do configure the `walletWrapper`, but instead of making it available as export like the `chainWrapper` and `contractWrapper`, we pass it directly into the contract wrapper, which will make use of it where needed.


## Behind the DataHandler: Interacting with the blockchain

So far, we stopped to dig deeper into the data flow in this Section when reaching the DataHandler, because this is our interface object, hiding the blockchain complexity behind it. To understand the full stack flow and the integrations with the other SDKs and frameworks, we need to look further, behind the DataHandler into the wrappers.

We still differentiate between two different types of interactions with the blockchain: __Queries__ and __transactions__. Since we just looked into user wallet interaction, we will follow up and begin with looking into how transactions are handled.
