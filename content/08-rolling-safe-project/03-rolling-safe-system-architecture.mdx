...
We also used this common interface to communicate (specification) updates between the smart contract development and the application. Using automated units tests which run on any code update, any breaking change will be immediately visible. This is very important for any project, because even with the best planning, it is natural that you might need to adjust your interface signatures as some details evolve during the course of a project.
