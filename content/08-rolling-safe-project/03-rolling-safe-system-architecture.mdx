...
We also used this common interface to communicate (specification) updates between the smart contract development and the application. When using automated units tests that run on any code update, any breaking change will be immediately visible. This is very important for any project because even with the best planning, it is natural that you might need to adjust your interface signatures as some details evolve during a project.

We will look into the details of the unit and integration tests in the [testing Section](./07-testing). You can take a look at the configuration and run results in our [public repository on GitHub](https://github.com/b9lab/tezos-app-project/actions).

## Two parts, two repositories

To further untangle the smart contract implementation details from the web app, we have also split up the projects into two separate repositories:
