...
Before working on this section, make sure to go through the [introduction section](./what-is-a-rolling-safe) to understand the underlying mechanisms of this novel funding instrument.

For a close look at the formulas used for calculation, we recommend consulting the [Continuous Organizations Whitepaper](https://github.com/C-ORG/whitepaper#annex) by Thibauld Favre. In this section, we will present an implementation of the whitepaper's ideas, our **TZMINT project**.

</HighlightBox>

We implemented our smart contract in [SmartPy](https://smartpy.io/). The tokens are represented as a simple map to keep things simple. In addition, we will store everything we want to keep track of on-chain:
...
In this section, we will begin with an explanation of the `buy` and `pay` functions. Afterwards, in the [next section](./rolling-safe-smart-contract-implementation-part-2), we will talk about the `sell` and `close` functions (also known as closing function). 
...
If these phases do not sound familiar, take another look at the [introduction section](./what-is-a-rolling-safe) elaborating on Rolling SAFEs, especially _Stages in a Rolling SAFE_.
...
After the calculation, `buy_initial` will check if any tokens can be issued. Then it will add the calculated number of tokens to the ledger linking them to the transaction sender. In case no record of the user exists, it will create an entry. The comments in the code should suffice as an explanation:
...
Next, the contract does a **reserve calculation** to determine the amount of the excess, which is sent back in the next lines:
...
_Why do we differentiate when it comes to excess?_
...
A customer can call the `pay()` entrypoint to send a payment to the organization via the smart contract. If `pay()` is called, the smart contract will issue new tokens and by default send them to the organization. Optionally, the customer can pass an address to which the tokens will be sent. A pre-determined part of the received tez will be kept in the reserve and the rest will be sent to the organization.
...
You can see, it will mint the same number of tokens as if `buy()` would have been called. A part of the tez sent with the call will be kept in the contract:
