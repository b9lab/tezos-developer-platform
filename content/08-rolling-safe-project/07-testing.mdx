...
Remember that we expect the `sell` call to fail before reaching the minimum funding goal (MFG), so to say pre-MFG.
...
We expect the price to increase after some `buy` calls take place.
