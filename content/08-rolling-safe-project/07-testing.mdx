...
We use [Chai](https://www.chaijs.com/) for unit tests and we have different unit tests for each wrapper. The `walletWrapper` is very small, so there is not much to test besides `callCount`s of the inner functions. This applies also to the unit tests for the `contractWrapper`.
...
In addition we have the possibility to pass the real network configuration and test the return types:
...
Integration Tests for the `walletWrapper` and the `contractWrapper` are need annother approch because of the browser extentions which are needed to use a wallet.