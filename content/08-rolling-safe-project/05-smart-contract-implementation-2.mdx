...
In the initial phase, one can sell a token and get 100% of the buy price, which will remain the same till MFG is reached.
...
If you want to take a look at the closing phase again, the [introduction section of this chapter](./what-is-a-rolling-safe) includes a section on _Stages in a Rolling SAFE_.
...
Only the contract owner can call it `close`. For the `close` transaction to be successful, the contract owner needs to send the exit fee. Note that the minimum period of time (MPT) must be reached if the organization wants to trigger the closing.
