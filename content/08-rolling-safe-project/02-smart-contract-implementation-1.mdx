...
Before working on this section, please read the [Continuous Organizations Whitepaper](https://github.com/C-ORG/whitepaper) by Thibauld Favre. In this section we will show the implementation of the ideas in the whitepaper. Please visit [TZMINT](https://tzmint.b9lab.com/) which is hosted by us and runs the implementation presented here. It is better to first try out the platform before coming back here. 

</HighlightBox>

We did implement our Smart Contract in SmartPy. To keep things simple, the tokens are represented as a simple map.

The contract will have three entrypoints:

- `buy`
- `sell`
- `close`

In this section, we will begin with an explanation of the `buy` function. Afterwards, in the [next section](...), we will talk about the `sell` function and the `close` (closing function). For the sake of simplicity, we will not implement the entrypoints for `burn` and `pay`. 
...
For the initial phase, you will find following lines in the contract code:
...
Before calculating the recent value of the total investments, the excess is calculated sent back to the user.
...
Next it will do a reserve-calculation to determine the amount of the excess, which we send back in the next lines:
...
Each time someone buys a token, a part of the tez amonunt in the transaction will be kept for the buy back reserve. 

<HighlightBox type="info">

At the end of a `buy_slope` call, the contract will update the sell slope. This is something we address in the [next section](...).
...
You can see, that we do not allow any action after the closing phase. Another thing is that we do an additional check for the excess, which we send back. The point is that the excess in `buy` functionn is a diffrent one than the excess in the `buy_initial` and `buy_slope` functions. We basicly do not want the user mix the initial and the slopy phase so the contract will allow to buy tokens till the MFG and the user will need another call to continue to buy in the slopy phase. 