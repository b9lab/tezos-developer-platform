...
The database container uses [PostgreSQL](https://www.postgresql.org/) to store the fetched data. It contains a database for the web application and a database for the TzKT indexer.
...
At the beginning of a test, Cypress opens a Google Chrome instance loading a previously downloaded version of the wallet extension. When the test requires interaction with the wallet, it will import the account via a mnemonic and configure the settings for the custom-created network.
