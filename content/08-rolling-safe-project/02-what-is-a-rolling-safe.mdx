...
The **`buy()` function** is called by the investor to buy the Rolling SAFE tokens from the DAT. The DAT issues new Rolling SAFE tokens in proportion to the investment amount received. The DAT uses the `pay()` function to send a portion of the funds to the reserve and the balance into the issuers/companies wallet.

The **`sell()` function** is called by the investors to sell their Rolling SAFE tokens. The DAT receives the Rolling SAFE tokens, burns them, and uses the `pay()` function to send back the corresponding amount of stablecoins held by the reserve to the investor as payment.

As seen above the **`pay()` function** sends funds from one party to the other passing through the DAT and to the reserve.

<!-- What do you mean by "passing through the DAT"? Please rewise wording. -->
