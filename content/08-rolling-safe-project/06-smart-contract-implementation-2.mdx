...
In the initial phase, one can sell a token and get 100% of the buy price, which will remain the same until the MFG is reached. We do not use the `sell_slope` in the initial phase, because the price is fixed but we will keep it updated. 

In the post-MFG phase, we will use the `sell_slope` to calculate the amount of tez to be sent:
...
In the formula, our `pay_amount` represents `M`, `x` is the amount of tez sent with the transaction, `x'` is the amount of burned tokens and `a` represents the total amount of tokens issued.

As you can see, the sell slope will increase if tokens are burned but the buy slope is always constant. This means that the entrypoint `burn`, allows for the organization to increase the sell price.

You can see that the contract `burn`s the amount of tokens sold. 

## Calculating the sell slope

Compared to the buy price calculation, this time it is a bit more complex because we do not have a linear price function for the sell price. The sell slope will change over time. The sell slope depends on the reserve, which is represented by the contract balance:
...
The value of `sp.balance` is the balance of the contract __after the transaction is received but before the contract sends any tez back to the user__. So the helper function needs to know the amount of tez sent back.
...
In addition the contract offers a `burn` entrypoint, in case a token holder wants to burn it's tokens:
...
Notice that if the data in the ledger increases in size, closing will cost more beacuse of:
