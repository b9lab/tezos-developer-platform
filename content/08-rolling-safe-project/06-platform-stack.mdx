...
First, we can see a lot in the storage fetched information and we do not always expect such a value to change:
...
To fetch user interactions, we need to first fetch the transactions with `transactions` and then the storage with `requestStorage` to get the information on the ledger change and the tez amount sent in the transaction.
