...
The component that brings the platform and the Tezos blockchain together is the `DataHandler`. It requests the data the platform needs with `chainWrapper` and uses the `contractWrapper` to buy and sell tokens. 